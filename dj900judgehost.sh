...
#pascal
#sudo apt install fp-compiler -y
#JavaScript
...
wget https://raw.githubusercontent.com/justiceHui/domjudge-installer/main/dj900start.sh
sudo sed -i "s/\$USER/${USER}/g" ~/dj900start.sh

#Number of judgehosts autoscaling for the number of cpu change to /etc/rc.local
echo '#!/bin/bash' >> ~/rc.local
echo "bash /home/${USER}/dj900start.sh" >> ~/rc.local
...
if [ -e /etc/default/grub.d/50-cloudimg-settings.cfg ]; then
  ...
  sudo sed -i "s#GRUB_CMDLINE_LINUX_DEFAULT=\"console=tty1 console=ttyS0 nvme_core.io_timeout=4294967295\"#GRUB_CMDLINE_LINUX_DEFAULT=\"console=tty1 console=ttyS0 nvme_core.io_timeout=4294967295 quiet cgroup_enable=memory swapaccount=1 isolcpus=2\"#" /etc/default/grub.d/50-cloudimg-settings.cfg
fi
...
#https://www.domjudge.org/docs/manual/9.0/judging.html
