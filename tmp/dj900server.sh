...
#Database location: local(default) or external machine
DBMODE="x"
while [ ${DBMODE} != "local" ] && [ ${DBMODE} != "external" ]; do
  clear
  echo    ""
  echo    "Where will MariaDB run?"
  echo    "local    : install MariaDB on this machine (single-machine setup)"
  echo    "external : connect to MariaDB on a separate machine"
  echo    "           (run dj900db.sh on that DB machine first)"
  echo    ""
  echo -n "Database location? [local/external]: "
  read DBMODE
done
...
case ${DBMODE} in
  ...
  "external")
    DBHOSTIN="o"
    INPUTS="x"
    while [ ${DBHOSTIN} != ${INPUTS} ]; do
      echo ""
      echo "Input the external DB machine's host (private IP or DNS name)."
      echo "Examples:"
      echo "10.0.0.20"
      echo "db.contest.domjudge.org"
      echo ""
      echo -n "Input  DB host : "
      read DBHOSTIN
      echo -n "Repeat DB host : "
      read INPUTS
    done
    DBHOST="${DBHOSTIN}"
    echo ""
    echo "External DB host set to: ${DBHOST}"
    echo "Make sure dj900db.sh has been run on that machine and port 3306 is reachable."
    echo ""
    ;;
esac
...
case ${DBMODE} in
  "local")
    ...
    ;;
  "external")
    #MariaDB lives on ${DBHOST}; install only the client utilities
    sudo apt install mariadb-client -y
    echo ""
    echo "Skipping local MariaDB install. Using external DB at ${DBHOST}."
    echo ""
    ;;
esac
